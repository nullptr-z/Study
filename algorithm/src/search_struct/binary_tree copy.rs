...
fn generate_binary_tree<T: Copy + PartialEq>(
    prev: &[T],
    center: &[T],
    ...
    while root.is_some() && i > 0 {
        ...
        if flag {
            ...
            if left_len < len {
                ...
                let (len, result) =
                    generate_binary_tree(&prev[left_len..len], &center[center_start..len]);
...
// let preorder = ['G', 'D', 'H', 'B', 'E', 'A', 'F', 'I', 'C'];
// let inorder =  ['G', 'H', 'D', 'E', 'B', 'I', 'F', 'C', 'A'];
...
fn generate_binary_tree_middle_back2<T: Copy + PartialEq>(
    ...
    let mut vec = Vec::with_capacity(100);
    let mut divider_vec = FIFO::with_capacity(middle_len * 2);

    divider_vec.push(back[back_len - 1]);
    vec.push(back[back_len - 1]);

    let root = Some(Rc::new(RefCell::new(TreeNode::new(*back.last().unwrap()))));
    while divider_vec.len() > 0 {
        // 通过中序的位置,找到new 后根
        let dv = divider_vec.pop().unwrap();

        let divide_l = get_index(middle, &dv).unwrap();
        let root_value = *back.get(divide_l - 1).unwrap();
        divider_vec.push(root_value);
        ...
        let divide_r = get_index(back, &dv).unwrap();
        let divider_r = back[divide_r - 1];
        divider_vec.push(divider_r);
    }
    let a = 1;
    root
}
...
fn test_binary_tree() {
    ...
    let (len, result) = generate_binary_tree(&preorder, &inorder);
